...
use crate::hadalyth_twitch_resources::TwitchApiEventSubs;
...
pub async fn init_device_user_token_async(
    ...
    let Ok(device_code_response) = device_code_response else {
        ...
        let error = format!("device_token_exchange_response bad {}", device_code_response.unwrap_err());
    ...
    let _ = tx.send(TwitchEvent::DeviceUserTokenRequest(verification_uri.clone(), expires_in));

    // Wait for the response to be processed

    let device_token_exchange_response = token_builder.wait_for_code(&http_client, tokio::time::sleep).await;
    let Ok(device_token_exchange_response) = device_token_exchange_response else {
        let _ = tx.send(TwitchEvent::DeviceUserTokenStatus(None));
        let _ = tx.send(TwitchEvent::Debug("device token exchange response bad".to_string()));
        return;
    };

    let _ = tx.send(TwitchEvent::DeviceUserTokenStatus(Some(device_token_exchange_response)));
    ...
    while let Some(message) = socket_read.next().await {
        ...
        match message {
            ...
            tokio_tungstenite::tungstenite::Message::Text(payload) => {
                ...
                match event_data {
                    twitch_api::eventsub::EventsubWebsocketData::Welcome { metadata : _, payload } => {
            
                        // Process welcome and emit the session id back into the main thread so event subs can be issued
                        let _ = tx.send(TwitchEvent::TwitchSocketStatus(Some(payload.session.id.to_string())));
            
                    }
                    twitch_api::eventsub::EventsubWebsocketData::Reconnect { metadata : _, payload } => {
                        ...
                        let Some(reconnect_url) = reconnect_url else {
                            let _ = tx.send(TwitchEvent::TwitchSocketStatus(None));
                            let _ = tx.send(TwitchEvent::Debug("Reconnect with no valid reconnect url".to_string()));
                            return;
                        };

                        // Replace the socket with a new one
                        let socket = tokio_tungstenite::connect_async(reconnect_url.to_string()).await;
                        let Ok(socket) = socket else {
                            let _ = tx.send(TwitchEvent::TwitchSocketStatus(None));
                    ...
                    twitch_api::eventsub::EventsubWebsocketData::Keepalive { metadata : _, payload : _ } => {
                        
                        // TODO : Reset a timer somewhere

                    }
                    twitch_api::eventsub::EventsubWebsocketData::Revocation { metadata : _, payload : _ } => {
                    ...
                    twitch_api::eventsub::EventsubWebsocketData::Notification { metadata : _, payload } => {
...
pub async fn init_subscribe_eventsubs_async(
    ...
    if eventsubs.automod_message_hold_v2 {
        ...
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::automod::AutomodMessageHoldV2::new(token.user_id.clone(), token.user_id.clone()),
    ...
    if eventsubs.automod_message_update_v2 {
        ...
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::automod::AutomodMessageUpdateV2::new(token.user_id.clone(), token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.automod_settings_update_v1 {
        ...
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::automod::AutomodSettingsUpdateV1::new(token.user_id.clone(), token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.automod_terms_update_v1 {
        ...
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::automod::AutomodTermsUpdateV1::new(token.user_id.clone(), token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_ad_break_begin_v1 {
        ...
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelAdBreakBeginV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_bits_use_v1 {
        ...
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelBitsUseV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_chat_clear_v1 {
        ...
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelChatClearV1::new(token.user_id.clone(), token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_chat_clear_user_messages_v1 {
        ...
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelChatClearUserMessagesV1::new(token.user_id.clone(), token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_chat_message_v1 {
        ...
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelChatMessageV1::new(token.user_id.clone(), token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_chat_message_delete_v1 {
        ...
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelChatMessageDeleteV1::new(token.user_id.clone(), token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_chat_notification_v1 {
        ...
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelChatNotificationV1::new(token.user_id.clone(), token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_chat_user_message_hold_v1 {
        ...
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelChatUserMessageHoldV1::new(token.user_id.clone(), token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_chat_user_message_update_v1 {
        ...
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelChatUserMessageUpdateV1::new(token.user_id.clone(), token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_chat_settings_update_v1 {
        ...
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelChatSettingsUpdateV1::new(token.user_id.clone(), token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_charity_campaign_donate_v1 {
        ...
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelCharityCampaignDonateV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_charity_campaign_start_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelCharityCampaignStartV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_charity_campaign_stop_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelCharityCampaignStopV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_update_v2 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelUpdateV2::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_follow_v2 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelFollowV2::new(token.user_id.clone(), token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_subscribe_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelSubscribeV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_cheer_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelCheerV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_ban_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelBanV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_unban_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelUnbanV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_unban_request_create_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelUnbanRequestCreateV1::new(token.user_id.clone(), token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_unban_request_resolve_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelUnbanRequestResolveV1::new(token.user_id.clone(), token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_warning_acknowledge_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelWarningAcknowledgeV1::new(token.user_id.clone(), token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_warning_send_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelWarningSendV1::new(token.user_id.clone(), token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_points_custom_reward_add_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelPointsCustomRewardAddV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_poll_begin_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelPollBeginV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_poll_progress_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelPollProgressV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_poll_end_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelPollEndV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_prediction_begin_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelPredictionBeginV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_prediction_progress_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelPredictionProgressV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_prediction_lock_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelPredictionLockV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_prediction_end_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelPredictionEndV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_raid_v1 {
        ...
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelRaidV1::to_broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
        ...
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelRaidV1::from_broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_shared_chat_begin_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelSharedChatBeginV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_shared_chat_end_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelSharedChatEndV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_shared_chat_update_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelSharedChatUpdateV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_shield_mode_begin_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelShieldModeBeginV1::new(token.user_id.clone(), token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_shield_mode_end_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelShieldModeEndV1::new(token.user_id.clone(), token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_shoutout_create_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelShoutoutCreateV1::new(token.user_id.clone(), token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_shoutout_receive_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelShoutoutReceiveV1::new(token.user_id.clone(), token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_suspicious_user_message_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelSuspiciousUserMessageV1::new(token.user_id.clone(), token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_suspicious_user_update_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelSuspiciousUserUpdateV1::new(token.user_id.clone(), token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_goal_begin_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelGoalBeginV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_goal_progress_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelGoalProgressV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_goal_end_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelGoalEndV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_hype_train_begin_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelHypeTrainBeginV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_hype_train_progress_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelHypeTrainProgressV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_hype_train_end_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelHypeTrainEndV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_moderate_v2 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelModerateV2::new(token.user_id.clone(), token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.stream_online_v1 {
        ...
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::stream::StreamOnlineV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.stream_offline_v1 {
        ...
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::stream::StreamOfflineV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_subscription_end_v1 {
        ...
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelSubscriptionEndV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_subscription_gift_v1 {
        ...
        // channel.bits.use
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelSubscriptionGiftV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));
    ...
    if eventsubs.channel_subscription_message_v1 {
        let result = twitch.create_eventsub_subscription(
            twitch_api::eventsub::channel::ChannelSubscriptionMessageV1::broadcaster_user_id(token.user_id.clone()),
            twitch_api::eventsub::Transport::websocket(&session_id), 
            &token.clone()
        ).await;
        let _ = tx.send(TwitchEvent::Debug(format!("{:?}", result)));       
