        ...
        while let Ok(event) = self.rx.try_recv() {
            match event {
                ...
                TwitchEvent::DeviceUserTokenRequest(verification_uri, expires_in) => {
                    godot_print!("TwitchEvent::DeviceUserTokenRequest: {}, {}", verification_uri, expires_in);
...
impl HadalythTwitch {
    ...
    #[signal] fn device_user_token_status(status : bool);
    #[signal] fn twitch_socket_status(status : bool);
...
impl HadalythTwitch {
    ...
    fn init_device_user_token(&mut self) {
        ...
        let token_builder = twitch_api::twitch_oauth2::DeviceUserTokenBuilder::new(client_id, scopes);
        

        runtime.spawn(
            init_device_user_token_async(tx, http_client, token_builder)
        );
    ...
    fn init_twitch_socket(&mut self) {
        ...
        if self.socket_tx.is_some() {
            let _ = self.socket_tx.as_ref().unwrap().send(SocketEvent::SocketClose);
            self.socket_tx = None;
        }
    ...
    fn init_subscribe_eventsubs(&mut self) {
        ...
        runtime.spawn(
            init_subscribe_eventsubs_async(
                tx,
                twitch, 
                token, 
                session_id, 
