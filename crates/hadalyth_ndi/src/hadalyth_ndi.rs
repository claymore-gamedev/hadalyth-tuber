...
use crate::custom_events::ndi_event::NdiEvent;
...
impl ITextureRect for HadalythNdi {
    ...
    fn process(&mut self, _delta: f32) {
        ...
        while let Ok(event) = self.rx.try_recv() {
            match event {
                NdiEvent::SourcesFound(sources) => {
                    ...
                    let Ok(framesync) = framesync else {
                        self.framesync = None;
                        return;
                    };
        ...
        let (timecode, height, width, data, data_len) = {
            ...
            let Ok(video) = framesync.capture_video(grafton_ndi::ScanType::Progressive) else {
                return;
            };
        ...
        if self.recv_frames == 0 {
            ...
            self.image_buffer =
                PackedByteArray::from(unsafe { std::slice::from_raw_parts(data, data_len) });

            // Create an image texture
            let image =
                Image::create_from_data(width, height, false, Format::RGBA8, &self.image_buffer);
        ...
        // Update the current image texture
            ...
            unsafe {
                std::ptr::copy_nonoverlapping(
                    data,
                    self.image_buffer.as_mut_slice().as_mut_ptr(),
                    data_len,
                );
            }
            ...
            RenderingServer::singleton().texture_2d_update(self.image_rid, image, 0);
        }
        self.recv_frames += 1;
    }
}
